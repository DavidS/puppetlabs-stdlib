...
Puppet::Functions.create_function(:'stdlib::fqdn_rand_string') do
  ...
  def default_impl(*args)
    ...
    raise(ArgumentError, 'fqdn_rand_string(): first argument must be a positive integer') unless call_function('stdlib::is_integer', args[0]) && args[0].to_i > 0
    ...
    for current in 1..length # rubocop:disable Style/For : An each loop would not work correctly in this circumstance
      rand_string << charset[call_function('fqdn_rand', charset.size, (args + [current.to_s]).join(':')).to_i]
    end
