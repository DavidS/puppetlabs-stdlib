...
describe 'stdlib::ensure_packages' do
  ...
  it { is_expected.to run.with_params.and_raise_error(ArgumentError, "'stdlib::ensure_packages' expects between 1 and 2 arguments, got none") }
  it { is_expected.to run.with_params('one', 'two', 'three').and_raise_error(ArgumentError, "'stdlib::ensure_packages' expects between 1 and 2 arguments, got 3") }
  it { is_expected.to run.with_params(1).and_raise_error(ArgumentError, "'stdlib::ensure_packages' parameter 'packages' expects a value of type Hash, Array, or String, got Integer") }
  it { is_expected.to run.with_params(['packagename', 1]).and_raise_error(ArgumentError, %r{rejected: parameter 'packages' variant 1 index 1 expects a String value, got Integer}) }
  ...
  context 'when given an empty packages array' do
    let(:pre_condition) { 'notify { "hi": } -> Package <| |>; $somearray = ["vim",""]; stdlib::ensure_packages($somearray)' }
